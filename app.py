...
st.markdown("""
    ...
    .metric-val { 
        font-size: 64px; 
        font-weight: 300; /* Sleek thin look */
        color: #0f172a; 
        line-height: 1.0;
        letter-spacing: -1px;
    }
...
# --- 2. MAX-LEGIBILITY CHART ENGINE (Thin & Big) ---
def apply_pro_layout(fig, title, chart_type="pie"):
    fig.update_layout(
        ...
        legend=dict(
            ...
            xanchor="center", 
            x=0.5,
            font=dict(size=20, color="#1e293b", family="Inter"), # Bigger legend
            itemwidth=60
        )
    )
    ...
    if chart_type == "bar":
        ...
        fig.update_yaxes(showgrid=False, tickfont=dict(size=20))
        fig.update_traces(
            marker_line_width=0, 
            opacity=0.85, 
            texttemplate='%{x}', 
            textfont=dict(size=30, color="#0f172a", family="Inter Thin"), # Thin & Big
        ...
        fig.update_traces(
            ...
            hole=0.6, # Thinner donut ring
            textfont=dict(size=22, family="Inter", color="white", weight=300), # Thin text
    ...
    try:
        ...
        st.markdown(f"""
            <div style="margin-bottom: 50px; border-left: 14px solid #3b82f6; padding-left: 35px; margin-top: 40px;">
                <h2 style="color: #0f172a; font-weight: 300; margin-bottom: 0px; font-size: 56px; letter-spacing: -2px; line-height: 1.0;">
