...
st.markdown("""
    ...
    .metric-container { background: #0d1117; border: 1px solid #30363d; border-left: 5px solid #58a6ff; padding: 18px; border-radius: 12px; margin-bottom: 10px; }
    ...
    .section-box { padding: 10px 0px; border-bottom: 1px solid #30363d; margin-bottom: 20px; color: #58a6ff; font-weight: 800; text-transform: uppercase; font-size: 13px; }
    </style>
...
if uploaded_file:
    try:
        ...
        with c1:
            ...
            fig_sev = px.bar(sev_data, x='Wks', y='Category', color='Severity', title="Max Longevity by Severity", orientation='h', barmode='group')
        ...
        with c2:
            ...
            fig_status = px.pie(status_counts, values='Count', names='Status', hole=0.6, title="Delivery Pipeline Status")
            st.plotly_chart(apply_executive_theme(fig_status), use_container_width=True)

        # --- ROW 3: DETAILED OPERATIONS LEDGER (The fix is here) ---
        st.markdown("<div class='section-box'>Detailed Operations Ledger</div>", unsafe_allow_html=True)
        
        # Triple-donut row
        d1, d2, d3 = st.columns(3)
        with d1:
            fig1 = px.pie(df, names='Status', hole=0.7, title="Delivery Pipeline Status")
            st.plotly_chart(apply_executive_theme(fig1), use_container_width=True)
        with d2:
            fig2 = px.pie(df, names='Severity', hole=0.7, title="Risk Distribution",
                          color='Severity', color_discrete_map={'Critical': '#f85149', 'High': '#d29922', 'Medium': '#58a6ff'})
            st.plotly_chart(apply_executive_theme(fig2), use_container_width=True)
        with d3:
            fig3 = px.pie(df, names='Domain', hole=0.7, title="Incident Volume")
        ...
        st.dataframe(
            ...
            column_config={"Wks": st.column_config.ProgressColumn("Longevity", min_value=0, max_value=52, format="%d weeks")},
    ...
    st.info("System Ready. Upload 'IT Operations' file.")
