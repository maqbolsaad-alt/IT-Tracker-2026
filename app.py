...
st.markdown("""
    ...
    .metric-val { 
        font-size: 58px; /* Increased for maximum clarity */
        font-weight: 900; 
        color: #0f172a; 
        line-height: 1.0;
        letter-spacing: -2px;
...
# --- 2. UPDATED CHART STYLING ENGINE (BIGGER & CLEARER) ---
def apply_pro_layout(fig, title, chart_type="pie"):
    fig.update_layout(
        template="plotly_white",
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        margin=dict(t=80, b=120, l=10, r=10), # Ample space for legends
        height=500, 
    ...
    if chart_type == "bar":
        fig.update_xaxes(showgrid=True, gridcolor="#f1f5f9", tickfont=dict(size=16, color="#64748b"))
        fig.update_yaxes(showgrid=False, tickfont=dict(size=18, color="#1e293b", family="Inter-SemiBold"))
        fig.update_traces(
            marker_line_width=0, 
            opacity=0.9, 
            texttemplate='<b>%{x}</b>', 
            textfont=dict(size=22, color="#1e293b"), # Large bar numbers
        ...
        fig.update_traces(
            ...
            hole=0.5, 
            textfont=dict(size=18, family="Inter-Bold"), # Clearer pie numbers
        ...
        fig.update_layout(
            legend=dict(
                orientation="h", 
                yanchor="top", 
                y=-0.2, # Push legend away from chart
                xanchor="center", 
                x=0.5,
                font=dict(size=16) # Readable legend text
            )
        )
    return fig
...
if uploaded_file:
    try:
        ...
        st.markdown(f"""
            <div style="margin-bottom: 50px; border-left: 12px solid #3b82f6; padding-left: 30px; margin-top: 35px;">
                <h2 style="color: #0f172a; font-weight: 900; margin-bottom: 0px; font-size: 52px; letter-spacing: -2px; line-height: 1.0;">
                    IT Tracker Dashboard
                </h2>
                <p style="color: #64748b; font-size: 20px; text-transform: uppercase; letter-spacing: 4px; margin-top: 12px; font-weight: 700;">
